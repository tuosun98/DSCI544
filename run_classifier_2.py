...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in range(int(args.num_train_epochs)):
        ...
        print(" ")
        print('epoch',_+1,': loss:',cal_loss/len(train_dataloader))
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for row in range(preds.shape[0]):
            prob0 = math.exp(preds[row][0])
            pred_pro.append(prob0)
        if 'cuda' in str(args.device):
            torch.cuda.empty_cache()
        ...
        result = compute_metrics(eval_task, preds, out_label_ids,pred_pro)
...
def predict(args, model, tokenizer, prefix=""):
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        ...
        if args.output_mode == "classification":
            ...
            preds = np.squeeze(preds)
        assert len(preds)==len(pred_pro),'lengths of the two output columns are not equal'
        pred_data = {'pred':pd.Series(preds),
                     'pred_pro':pd.Series(pred_pro)}
        pred_data = pd.DataFrame(pred_data)
        #print(pred_data.head(10))
        output_path = os.path.join(pred_output_dir, prefix, 'pred_results.tsv')
        output_path2 = os.path.join(args.data_dir, 'pred_results.tsv')
        pred_data.to_csv(output_path,header=True,index=False,sep='\t')
        pred_data.to_csv(output_path2,header=True,index=False,sep='\t')
        # print prediction performance
        y_true_path = os.path.join(args.data_dir, prefix, 'test.tsv')
        y_true = pd.read_csv(y_true_path, sep='\t').label.tolist()
        y_pred = preds
        print('\n----- Prediction Performance -----\n')
        print('pearsonr: {:.4f}'.format(pearsonr(y_true, y_pred)[0]))
        print('spearmanr: {:.4f}'.format(spearmanr(y_true, y_pred)[0]))

        # write txt file
        write_path = os.path.join(pred_output_dir, prefix, 'pred_performance.txt')
        with open(write_path, 'w') as f:
            f.write('V: {:.4f}\n'.format(pearsonr(y_true, y_pred)[0]))
            f.write('spearmanr: {:.4f}\n'.format(spearmanr(y_true, y_pred)[0]))
...
def main():
    ...
    parser.add_argument('--eval_best_model',type=str,default='f1',
                        help="Evaluation criterias for choosing best model of all checkpoints,can be ['f1','acc','acc_and_f1','prec','recall','auc','eval_loss']")
    parser.add_argument('--save_best_model',default=True,
                        help="Whether to save the model which has the best performance,if False,save the final model")
    ...
    if args.do_train:
        ...
        global_step, tr_loss, train_results = train(args, train_dataset, model, tokenizer)
        logger.info(" global_step = %s, average loss = %s", global_step, tr_loss)
        # Create output directory if needed
        output_dir = os.path.join(args.output_dir, 'checkpoint-end')
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        #Save the final model to checkpoint-end
        model_to_save = model.module if hasattr(model, 'module') else model  # Take care of distributed/parallel training
        model_to_save.save_pretrained(output_dir)
        torch.save(args, os.path.join(output_dir, 'training_args.bin'))
        logger.info("Saving model end checkpoint to %s", output_dir)
        tokenizer.save_vocabulary(vocab_path=output_dir)

        #eval_results of all checkpoints
        result = evaluate(args, model, tokenizer)
        result = dict((k + '_end', v) for k, v in result.items())
        train_results.update(result)
        ...
        if args.save_best_model:
            ...
            for key in train_results.keys():
                if re.match('^%s_\w+'%eval_choice,key) is not None:
                    result = {key:train_results[key]}
                    eval_results.update(result)

            if eval_choice == "eval_loss":
                best_key = min(eval_results,key=eval_results.get)

            best_key = max(eval_results,key=eval_results.get)
            best_index = re.match('(^%s_)(\w+)'%eval_choice,best_key).group(2)
            best_checkpoint = 'checkpoint-'+best_index
            logger.info("Saving the best model at checkpoint%s", best_checkpoint)
            best_path = os.path.join(args.output_dir,best_checkpoint)

            for file_name in os.listdir(best_path):
                full_file_name = os.path.join(best_path, file_name)
                if os.path.isfile(full_file_name):
                    shutil.copy(full_file_name, args.output_dir)
        #Save the final model
        else:
            final_path = os.path.join(args.output_dir,'checkpoint-end')
            for file_name in os.listdir(final_path):
                full_file_name = os.path.join(final_path, file_name)
                if os.path.isfile(full_file_name):
                    shutil.copy(full_file_name, args.output_dir)


        output_eval_file = os.path.join(args.output_dir, "all_checkpoint_eval_results.txt")
        with open(output_eval_file, "w") as writer:
            for key in sorted(train_results.keys()):
                writer.write("%s = %s\n" % (key, str(train_results[key])))
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        if args.eval_all_checkpoints:
            checkpoints = list(os.path.dirname(c) for c in sorted(glob.glob(args.output_dir + '/**/' + WEIGHTS_NAME, recursive=True)))
            logging.getLogger("transformers.modeling_utils").setLevel(logging.WARN)  # Reduce logging
        logger.info("pridict the following checkpoints: %s", checkpoints)
        for checkpoint in checkpoints:
            global_step = checkpoint.split('-')[-1] if len(checkpoints) > 1 else ""
            prefix = checkpoint.split('/')[-1] if checkpoint.find('checkpoint') != -1 else ""
            model = model_class.from_pretrained(checkpoint)
